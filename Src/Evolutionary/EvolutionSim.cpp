...
void EvolutionSim::runEvolution(int generations, int populationSize) {
    ...
    for (int i = 0; i < populationSize; ++i) {
        Organism organism;
        organism.neuralNetwork = NeuralNetwork(networkShape);
    ...
    for (int i = 0; i < generations; ++i) {
        ...
        storeOutput(pop.at(0).neuralNetwork);
    }
}
...
std::vector<Organism> EvolutionSim::generateNewPopulation(const std::vector<Organism>& oldPopulation) {
    ...
    // Copy individuals based on exp distribution. Better individuals are more likely to be copied
...
void EvolutionSim::storeOutput(const NeuralNetwork& neuralNetwork) {
    // Saves output from neural network to text file for visualisation purposes
    std::fstream f;
    f.open("nnData.txt", std::fstream::trunc);
    for (int i = -25; i < 25; i++) {
        auto output = neuralNetwork.getOutput({(double)i * 4.0 / 100.0});
        for (auto x : output) {
            f << x << ", ";
        }
    }
